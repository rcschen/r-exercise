...
B = data.frame(x1=runif(5), x2=runif(5), x3=runif(5))
...
C
C=read.table("/tmp/foo.csv", header = FALSE, sep = ",", row.names=1)
C
C=read.table("/tmp/foo.csv", header = FALSE, sep = ",", row.names=NULL)
C
