...
print(paste0("r[i]=sin[i]:",r))
# The good way -- don't use loops --
print(paste0("sin(x):",sin(x)))
...
print(paste0("runif(1):", matrix(runif(1),2,3)))
print("----")
print(paste0("runif(6):", matrix(runif(6),2,3)))
#print(paste0("runif(9):", matrix(runif(9),2,3)))
x <- 1:20
m <- matrix(rep(x,2), ncol = 2)
print(paste0("the other matrix way: matrix( rep(x,2), ncol=2):"))
print(m)

N=4; M=100;
r = matrix(runif(N*M), N, M)
#print(paste0("matric(runif(n*m),n,m)",r))
#print(r)
...
print("m%*%t(m)")
print(m%*%t(m))
print("an example for c %*% m")
print(c(1,2)%*%m)
# wrong: print(m%*%t(c(1,2)))
...
s2
all.equal(ans1,ans2)
s1/s2           # On my machine it's 2000x fasteri
